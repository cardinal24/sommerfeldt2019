...
##psych_251_report_cleaned_wide <- read_csv("R Mac working folder/sommerfeldt2019/psych.251.report.cleaned.wide.csv")
#psych_251_report_cleaned_long <- read_csv("R Mac working folder/sommerfeldt2019/psych.251.report.cleaned.long.csv")
...
https://github.com/cardinal24/sommerfeldt2019
...
For this reproduction, the final N for analyses was 967. This is lower than original paper (n=1065) reported and is likely due to differing parameters of exclusion resulting in a difference of 98 participants. I removed participants that did not have five measures of heart rate and self-reported stress. Additionally, I did not change scores on the PWB measure to NA if they equaled 98 which was not reported in the paper but shown in the original paper script. I likely eliminated more participants in cleaning and transformations which may have impacted variables differently. 



## Results
In general, the results support the key hypotheses in the paper. Stress-heart rate coherence is significantly associated with measures of health and well-being. However, although trending the same way this was not significant for C-reactive protein. Below I report estimates, standard errors, degrees of freedom, F and P values.
...
#key variables to look at summary stats
pwb.summary <- describe(wDF$pwb2) 
depression.summary <- describe(wDF$P4_CESD)
anxiety.summary <- describe(wDF$P4_STAItrait)
IL6.summary <- describe(wDF$IL6_T) #different means and SDs than original paper for IL6 and CRP
CRP.summary <- describe(wDF$CRP_T)
denial.summary <- describe(wDF$COPE_denial)

kable(denial.summary)
...
```{r Linear-Mixed Model Results}
#summary of the mixed lmer models testing the main hypotheses, that stress-heart rate coherence is associated with improved measures of psychophysiological health and well-being


tab_model(lmerM.pwb, lmerM.dep, lmerM.anx, lmerM.IL6, lmerM.CRP, lmerM.den) 
```

Reproduction results - The results support the general conclusion of the paper

### Data preparation

Data preparation included thorough investigation of variables, cleaning, wrangling, and transforming (see cleaning.rmd for details). Data here are cleaned with scores of key variables. Participants were excluded if they did not complete stroop or MATH task for stressor task or if we did not have 5 measurements of heart rate and self-reported stress. 
...
Original author did not graph but listed the above numbers in a table.
...
## Discussion
Primarily, the results indicate that there is a significant relationship between heart rate and the interaction of stress and measures of health and well being. Specifically, individuals with higher stress-heart rate coherence also report higher psychological well being, fewer depressive symptoms, lower trait anxiety, IL-6 and are less likely to use denial as a coping strategy. Participants with higher stress-heart-rate coherence had lower CRP but this was non significant but trending in the same direction as the original paper.


### Summary of Reproduction Attempt

This was a successful reproduction for two reasons. 1)I was able to retrieve, prepare and analyze the data set using similar methodology, and 2) this resulted in similar results which can be deemed a successful reproduction. I reproduced the two main figures of the paper along with the models that reflect the main hypotheses. while the figures differ slightly, the overall relationships are significantly trending in the same direction and models produce similar estimates, F, and P values. Although we did differ on degrees of freedom listed in the models this is likely due to the population size and difference in cleaning/elimination methods that may have resulted in different population size. The summary statistics were similar dsepite a smaller sample, except for the for Interleukin-6 and C-reactive protein.

### Commentary
Overall, this author did an excellent job providing insight into their methods and approach and made this mostly available in an online repository. This greatly supported my reproductive endeavor and should be a critical part of open science.I only had one objection to a scoring method with the psychological well-being variable but this didn't result in meaningful differences. While the authors provided their code, there were no data to work from so I had the chance to practice retrieving and digging through a massive data set with 10k+ observers and variables. This process made me much more appreciative of providing clean data sets when choosing open science methods. I also walked away with a deeper appreciation of tidy cleaning methods and the importance of using the code, names, comments, and packages all together optimally which will be a learning process. When this is done well, it makes it incredibly easy for someone to understand thinking processes and reproduce work. I also gained better appreciation of the need for understanding how to clean, tarnsform and reshape data most effectively and how this is the bread and butter of analyses.
