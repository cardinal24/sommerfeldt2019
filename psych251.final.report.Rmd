...
#psych_251_repro_f_Ldf_analysis <- read_csv("sommerfeldt2019/psych.251.repro.f.Ldf.analysis.csv")
#psych_251_repro_w_Ldf_analysis <- read_csv("sommerfeldt2019/psych.251.repro.w.Ldf.analysis.csv")

wDF <- psych_251_repro_w_Ldf_analysis
f.Ldf <- psych_251_repro_f_Ldf_analysis
```
...
final N for analyses is 967 which is lower than the paper reported. I likely eliminated some scores and people in cleaning.
- i removed all participants with missing scores in HR and stress. Also did not change the PWB scores from 98=0.
xxx - addendum for any other methods changes
## Results
...
```{r}
#summary of the mixed lmer models 


tab_model(lmerM.pwb, lmerM.den, lmerM.dep,lmerM.CRP,lmerM.IL6, lmerM.anx) 
```

Reproduction results - The results supportthe general conclusion of the paper
...
The primary result is that we do see a significant relationship between heart rate,stress, and key variables measures psychophysiological health and well-being. This was a successful reproduction in that I was ableto clean, transform, and analyze the data effectively and get similar results using thesame methods as the author.
...
```{r coherence_CRP}
#Run the test
lmerM.CRP<- lmerTest::lmer(hr_value ~ stress_CMC *CRP_MC + P4_age_MC*stress_CMC + (1 + stress_CMC|M2ID) + (1|M2FAMNUM.x), data = f.Ldf)
